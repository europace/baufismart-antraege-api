...
Please
use [![Authentication](https://img.shields.io/badge/Auth-OAuth2-green)](https://docs.api.europace.de/common/authentifizierung/authorization-api/)
to get access to the APIs. The OAuth2 client requires the following scopes:
...
Loan 1 and 2 are descided by DSL-Bank, because KfW-Bank products are handled by the mortage bank (background process).
Loan 3 is a consumer loan wich is optimizing the mortage conditions.
This loan will be decided by Hanseatic themselfs and only needed, if DSL-Bank will approve.
...
To receive all the shared proofs for an application please use
the [Unterlagen Push API](https://docs.api.europace.de/baufinanzierung/unterlagen/unterlagen-push-api/)
...
If you set the loan officer, she/he can edit the application and the contact information will be automatically shared with the
advisor in BaufiSmart.
...
If you don't use different europace users for loan officers identity, it is helpful to set contact details for the advisors as
information.

> This operation is not necessary, if your loan officers has an identity in europace (see [set loan officer](#set-loan-officer))
> and it is set on the application.
...
The loan providers reference can be used in communication between advisor and loan officer.
Typically, an account-number is used here, when available.
...
> Important! Please prefer [set state](#set-state-and-send-message) for communication with advisor,
> because states are more helpful for process reporting. This method should be your last choice.
...
If a counteroffer is created, the original offer will be declined and marked as 'ABGELEHNT'
and the new offer is automatically approved and marked as `UNTERSCHRIEBEN ` from the loan provider.

A single loan or a combination of loans can be offered as a counteroffer.
The `vertriebsProvisionGesamtAbsolut` (commission amount) must be without the europace-fee.
The europace-fee will be recalculated based on your new loan details and is contained in the new application.
